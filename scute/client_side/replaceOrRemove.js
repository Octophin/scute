...
let popup = function(
  ...
  if (options.classes) {
    popup.setAttribute("class", options.classes);
  }
  ...
  buttons.forEach(function(button) {
    ...
    // Also destroy the popup and the overlay if it gets added
    buttonElement.addEventListener("click", function() {
      popup.parentNode.removeChild(popup);
      lockScreenOverlay(false);
    });

    document.querySelector(".pop-up-buttons").appendChild(buttonElement);
  });
...
let triggerAction = function(e) {
  ...
  if (
    element.hasAttribute("data-popupWarning") ||
    element.hasAttribute("data-usermessage")
  ) {
...
let loadPreset = function(presetID) {
  Array.from(document.querySelectorAll("[data-preset-menu]")).forEach(function(
    menu
  ) {
...
let deletePreset = function() {
  let preset = document
    .querySelector("[data-preset-menu][data-selected]")
    .getAttribute("data-preset-menu");
...
function checkPresetName() {
  ...
  let presetName = document
    .querySelectorAll("[data-selectedForm]")[0]
...
function checkScriptName() {
  ...
  let scriptName = document
    .querySelectorAll("[data-selectedForm]")[0]
    .getElementsByTagName("input")[0]
    .value.trim();
  let scriptDescription = document
    .querySelectorAll("[data-selectedForm]")[0]
    .getElementsByTagName("input")[1]
    .value.trim();
  let scriptCommands = document
    .querySelectorAll("[data-selectedForm]")[0]
